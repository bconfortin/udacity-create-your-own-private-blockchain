...
class Blockchain {
            ...
            try {
                ...
                if (self.chain.length > 0) {
                    block.previousBlockHash = badPreviousHashOnPurpose ?
                        self.chain[self.chain.length - 1].hash.split("").reverse().join("") :
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                moment(currentTime).subtract(5, 'minutes') > timeOfMessage &&
                    reject("Time elapsed between signing the message and submitting a star can't be more than 5 minutes.");
                !bitcoinMessage.verify(message, address, signature) &&
                    reject('Failed to verify Bitcoin Message.');
                const block = self._addBlock(new Block({
                    address,
                    star,
                }));
                resolve(block);
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
           try {
               const blockByHash = self.chain.find(block => block.hash === hash) || null;
               resolve(blockByHash);
           } catch (e) {
               reject(e);
           }
    ...
    getBlockByHeight(height) {
        ...
        return new Promise((resolve, reject) => {
            try {
                const blockByHeight = self.chain.find(block => block.height === height) || null;
                resolve(blockByHeight);
            } catch (e) {
                reject(e);
            }
        });
    ...
    validateChain() {
        ...
        return Promise.all(self.chain.map(block => block.validate()))
            .then(validatedBlock => {
