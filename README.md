...
Hello! I have done some slight modifications to the BlockchainController.js.
Please let the team know that the routes to get a block by height and by hash are
the same ("/block/:param"), which means the route to get a block by hash is never called.

I also made every third and fifth blocks break so it's easier to test if the validation
of the blocks is working.

Thank you for taking the time to review this. Have an awesome week!
